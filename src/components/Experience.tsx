        ...
        <div className="relative border-l border-white/10 ml-4 md:ml-6 space-y-16">
          {experiences.map((exp, index) => (
            <div key={index} className="relative pl-10">
              {/* Timeline Dot */}
              <div className="absolute left-[-6px] top-2 w-3 h-3 rounded-full bg-blue-500 ring-4 ring-[#020617] z-20"></div>
              
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div>
                  <h3 className="text-2xl font-bold text-white mb-1">{exp.role}</h3>
                  <div className="text-blue-400 font-medium text-lg flex items-center gap-2">
                    <Briefcase size={18} />
              ...
              <div className="glass-card p-8 rounded-2xl relative z-10 hover:border-blue-500/30 transition-all">
                <ul className="space-y-4">
                  {exp.achievements.map((item, i) => (
                    <li key={i} className="flex gap-3 text-slate-300 leading-relaxed">
                      <span className="w-1.5 h-1.5 rounded-full bg-blue-500/50 mt-2.5 shrink-0"></span>
                      {item}
                    </li>
                  ))}
                </ul>
